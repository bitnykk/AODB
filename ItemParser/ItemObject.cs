...
namespace AODB.Common.RDBObjects
{
    public class Modifier
    {
        ...
        {
            if (!_modifiers.TryGetValue(function, out var mods))
            {
                mods = new List<Dictionary<FunctionOperator, object>>();
                _modifiers.Add(function, mods);
            }
    ...
    public class ItemEntry
    {
        ...
        public Dictionary<int, uint> UnkSkills0x06K0x1B = new Dictionary<int, uint>();

        public Dictionary<StatId, uint> Stats = new Dictionary<StatId, uint>();
        public Dictionary<SkillCheck, Dictionary<StatId, uint>> SkillChecks = new Dictionary<SkillCheck, Dictionary<StatId, uint>>();
        public Dictionary<EventType, Modifier> Modifiers = new Dictionary<EventType, Modifier>();
        public Dictionary<ActionType, Requirement> Requirements = new Dictionary<ActionType, Requirement>();

        public ItemEntry(byte[] blob)
        {
            Deserialize(blob);
        }
        ...
        {
            using (BinaryReader reader = new BinaryReader(new MemoryStream(blob)))
            {
                reader.ReadInt32();
                reader.ReadInt32();
                reader.ReadInt32();
        ...
        private void Read0x16Key0x24(BinaryReader reader)
        {
            ...
            if (!Requirements.TryGetValue(key, out Requirement req))
            {
                req = new Requirement();
                Requirements.Add(key, req);
            }

            for (int i = 0; i < count; i++)
                req.AddCriterion(key, new RequirementCriterion(reader.ReadInt32(), reader.ReadUInt32(), (Operator)reader.ReadInt32()));
        }
        ...
        {
            EventType eventType = (EventType)reader.ReadInt32();
            var count = reader.Read3F1();

            if (!Modifiers.TryGetValue(eventType, out Modifier mod))
            {
                mod = new Modifier();
                Modifiers.Add(eventType, mod);
            }
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                reader.ReadUInt32();
                reader.ReadUInt32();
                var criterionCount = reader.ReadUInt32();
                ...
                mod.AddModifier(spellFunction, operations);
            }
        }
            ...
            switch (key)
            {
                case 0x1b:
                    ...
                    for (int x = 0; x < count; x++)
                    {
                        ...
                        if (!UnkSkills0x06K0x1B.ContainsKey(unk1)) //156928 has x2 of the same unk skill
                            UnkSkills0x06K0x1B.Add(unk1, unk2);
                    }
                    break;
                default:
        ...
        {
            int skillTypes = reader.Read3F1();

            for (int x = 0; x < skillTypes; x++)
            {
                SkillCheck skillType = (SkillCheck)reader.ReadInt32();
                int skillCount = reader.Read3F1();

                if (!SkillChecks.TryGetValue(skillType, out var skills))
                {
                    skills = new Dictionary<StatId, uint>();
                    SkillChecks.Add(skillType, skills);
                }
                ...
                for (int y = 0; y < skillCount; y++)
                {
                    ...
                    if (!skills.ContainsKey(stat)) //156928 has x2 of the same def skill
