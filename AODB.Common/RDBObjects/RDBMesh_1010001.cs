...
namespace AODB.RDBObjects
{
    ...
    public class RDBMesh : RDBObject
    {
        ...
        public override void Deserialize(BinaryReader reader)
        {
            ...
            {
                for (int i = 0; i < rTriMeshT.data.Length; i++)
                {
                    var faftMeshData = (RDBMesh_t.FAFTriMeshData_t)RDBMesh_t.Members[rTriMeshT.data[i]];
                    ...
                    for (int simpleMeshIndex = 0; simpleMeshIndex < faftMeshData.mesh.Length; simpleMeshIndex++)
                    {
                        ...
                        if (matTexture != null)
                        {
                            var texCreator = (RDBMesh_t.AnarchyTexCreator_t)RDBMesh_t.Members[matTexture.creator];
